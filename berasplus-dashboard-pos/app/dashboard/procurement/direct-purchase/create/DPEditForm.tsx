...
export default function DPEditForm({
  ...
  const [purchaseDate, setPurchaseDate] = useState(
    initialData.purchase_date ? new Date(initialData.purchase_date).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10)
  )

  const [items, setItems] = useState(
    initialData.items.map(i => ({
      id: Date.now() + Math.random(),
      item_id: i.id, // original dp_item id
      product_id: i.product_id,
      quantity: i.quantity.toString(),
      price_per_unit: i.price_per_unit.toString()
    }))
  )

  const addItem = () => setItems([...items, { id: Date.now(), item_id: '', product_id: '', quantity: '', price_per_unit: '' }])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDirectPurchase(initialData.id, payload)
  ...
  return (
    <div className="max-w-4xl space-y-6 pb-20">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Edit Pembelian</h1>
          <p className="text-sm text-zinc-500">Ubah data pembelian yang masih dalam status Waiting Delivery</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex justify-end gap-4">
          <Link href={`/dashboard/procurement/direct-purchase/${initialData.id}`} className="px-6 py-2.5 rounded-xl text-sm font-semibold text-zinc-700 hover:bg-zinc-100">Batal</Link>
